...
export default function PropertyDetailsPage() {
  ...
  const handleFavoriteToggle = () => {
    ...
    toast.message(isFavorited ? "Removed from favorites" : "Added to favorites",{
  ...
  return (
    <div className="container py-8 max-w-6xl mx-auto">
      {/* Header */}
      <div className="mb-6">
        <Button variant="ghost" onClick={() => router.push("/land")} className="mb-4">
          <ArrowLeft className="mr-2 h-4 w-4" />
          Back to Properties
        </Button>
                    ...
                    <CarouselItem key={index}>
                      <div className="relative h-96">
                        <div className="absolute inset-0 bg-slate-200 flex items-center justify-center">
                          ...
                          <span className="ml-2 text-slate-500">Property Image {index + 1}</span>
                        </div>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center p-4 bg-slate-50 rounded-lg">
                  ...
                  <div className="font-bold text-2xl text-green-600">{formatPrice(property.price)}</div>
                  <div className="text-sm text-muted-foreground">Total Price</div>
                </div>
                <div className="text-center p-4 bg-slate-50 rounded-lg">
                  <Ruler className="h-8 w-8 mx-auto mb-2 text-blue-600" />
                  <div className="font-bold text-2xl">{property.landSize}</div>
                  <div className="text-sm text-muted-foreground">Acres</div>
                </div>
                <div className="text-center p-4 bg-slate-50 rounded-lg">
                  <Calendar className="h-8 w-8 mx-auto mb-2 text-purple-600" />
                  <div className="font-bold text-lg">{new Date(property.postedDate).toLocaleDateString()}</div>
                  <div className="text-sm text-muted-foreground">Posted Date</div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h3 className="font-semibold text-lg">{property.seller.name}</h3>
                  <p className="text-sm text-muted-foreground">
                    Member since {new Date(property.seller.joinedDate).getFullYear()}
                  </p>
                  <p className="text-sm text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span>{formatPrice(property.price / property.landSize)}</span>
              </div>
            </CardContent>
          </Card>
        </div>
